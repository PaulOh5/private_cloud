...
class UpdateUserHandler:
    ...
    async def handle(self, command: UpdateUserCommand) -> User:
        target = await self.user_repository.get_by_id(command.user_id)
        if not target:
        ...
        new_tenant_id = (
            command.tenant_id if command.tenant_id is not None else target.tenant_id
        )
        if new_role == "admin":
        ...
        if (
            ...
            and (new_role != "admin" or new_is_active is False)
        ):
            if await self.user_repository.count_active_admins() <= 1:
        ...
        if (
            command.role is not None
            or command.is_active is False
            or command.password is not None
        ):
            await self.refresh_token_repository.revoke_all_for_user(command.user_id)
        await self.audit_logger.write(
            action="user.update",
            target_type="user",
...
class DeactivateUserHandler:
    ...
    async def handle(self, command: DeactivateUserCommand) -> None:
        ...
        if (
            ...
            and await self.user_repository.count_active_admins() <= 1
        ):
            raise ConflictError("at least one active admin is required")
        if target.is_active:
            await self.user_repository.update_user(
                user_id=command.user_id, is_active=False
            )
            await self.refresh_token_repository.revoke_all_for_user(command.user_id)
            await self.audit_logger.write(
