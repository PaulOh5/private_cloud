...
from app.application.services.task_result_processor import RetryableResultEventError, TaskResultProcessor, VmResultEvent
            ...
            try:
                ...
                for method, _, body in channel.consume(self.RESULT_QUEUE, inactivity_timeout=1, auto_ack=False):
                    ...
                    try:
                        ...
                        logger.exception("failed to process vm result event")
                        channel.basic_ack(method.delivery_tag)
            except Exception:
