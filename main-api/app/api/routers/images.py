...
async def _sync_images_impl(request: Request) -> SyncVmImagesResponse:
    try:
...
async def sync_images(
    ...
    return await _sync_images_impl(request)


@legacy_image_router.post(
    "/sync", response_model=SyncVmImagesResponse, include_in_schema=False
)
async def sync_images_legacy_alias(
    request: Request,
    _=Depends(require_roles("admin")),
):
    return await _sync_images_impl(request)
